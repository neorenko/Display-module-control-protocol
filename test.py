...
class TestDisplayCommandParser(unittest.TestCase):
    ...
    def test_draw_text(self):
        text = "Hello"
        command = bytes([0x0C, 0x00, 0x32, 0x00, 0x64, 0xFF, 0xFF, 0x01, len(text)] + list(text.encode('utf-8')))
        result = self.parser.parse(command)
        self.assertIsNotNone(result, "Parse result should not be None")
        self.assertEqual(result['command_id'], 0x0C)
        self.assertEqual(result['x0'], 50)
        self.assertEqual(result['y0'], 100)
        self.assertEqual(result['color'], 0xFFFF)
        self.assertEqual(result['text'], "Hello")

    def test_invalid_draw_text_length(self):
        text = "Hello"
        command = bytes([0x0C, 0x00, 0x32, 0x00, 0x64, 0xFF, 0xFF, 0x01, len(text) + 1] + list(text.encode('utf-8')))
        result = self.parser.parse(command)
        self.assertIsNone(result, "Parse result should be None for invalid text length")
